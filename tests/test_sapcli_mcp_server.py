...
class TestAdtConnectionTest:
    """Tests for _adt_connection_test function."""

    def test_connection_test(self):
        """Test _adt_connection_test function."""
        mock_conn = MagicMock()

        output_buffer = server.OutputBuffer()
        sap.cli.core.set_console(output_buffer)

        server._adt_connection_test(mock_conn, None)

        mock_conn.collection_types.items.assert_called_once()

        assert output_buffer.capout == "ADT connection works!\n"
        assert output_buffer.caperr == ""


class TestAbapAdtConnectionTest:
    """Tests for abap_adt_connection_test MCP tool."""

    @patch('sap.adt.Connection')
    def test_tool_success(self, mock_connection, sample_adt_config):
        """Test abap_adt_connection_test tool success."""
        mock_conn = MagicMock()
        mock_connection.return_value = mock_conn

        result = server.abap_adt_connection_test.fn(**sample_adt_config)
        assert result.Success is True
        assert result.Contents == "ADT connection works!\n"
        assert result.LogMessages == ['']

    @patch('sap.adt.Connection')
    def test_tool_connection_failure(self, mock_connection, sample_adt_config):
        """Test abap_adt_connection_test tool with connection failure."""
        mock_conn = MagicMock()
        mock_connection.return_value = mock_conn
        mock_conn.collection_types.items.side_effect = SAPCliError('HTTP Timeout')

        result = server.abap_adt_connection_test.fn(**sample_adt_config)
        assert result.Success is False
        assert result.Contents == ""
        assert result.LogMessages == ['HTTP Timeout', '']


class TestAbapAdtPackageGetDetails:
    """Tests for abap_adt_package_get_details MCP tool."""

    @patch('sap.cli.package.stat')
    @patch('sap.adt.Connection')
    def test_tool_success(self, mock_connection, mock_stat, sample_adt_config):
        """Test abap_adt_package_get_details tool success."""
        mock_conn = MagicMock()
        mock_connection.return_value = mock_conn

        config_with_name = {**sample_adt_config, 'name': 'ZTEST_PACKAGE'}
        result = server.abap_adt_package_get_details.fn(**config_with_name)
        assert result.Success is True
        mock_stat.assert_called_once_with(mock_conn, SimpleNamespace(name='ZTEST_PACKAGE'))


class TestAbapAdtPackageListObjects:
    """Tests for abap_adt_package_list_objects MCP tool."""

    @patch('sap.cli.package.list_package')
    @patch('sap.adt.Connection')
    def test_tool_success_default_recursive(self, mock_connection, mock_list_package, sample_adt_config):
        """Test abap_adt_package_list_objects tool success without recursion."""
        mock_conn = MagicMock()
        mock_connection.return_value = mock_conn

        config_with_name = {**sample_adt_config, 'name': 'ZTEST_PACKAGE'}
        result = server.abap_adt_package_list_objects.fn(**config_with_name)
        assert result.Success is True
        mock_list_package.assert_called_once_with(mock_conn, SimpleNamespace(name='ZTEST_PACKAGE', recursive=False))


    @patch('sap.cli.package.list_package')
    @patch('sap.adt.Connection')
    def test_tool_success_recursive(self, mock_connection, mock_list_package, sample_adt_config):
        """Test abap_adt_package_list_objects tool success with recursion."""
        mock_conn = MagicMock()
        mock_connection.return_value = mock_conn

        config_with_name = {**sample_adt_config, 'name': 'ZTEST_PACKAGE', 'recursive': True}
        result = server.abap_adt_package_list_objects.fn(**config_with_name)
        assert result.Success is True
        mock_list_package.assert_called_once_with(mock_conn, SimpleNamespace(name='ZTEST_PACKAGE', recursive=True))
